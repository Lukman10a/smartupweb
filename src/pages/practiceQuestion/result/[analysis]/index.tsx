...
export default function Analysis() {
  ...
  useEffect(() => {
    if (studentTests) {
      const subjectFilter = studentTests.filter(
        (test) => test.course_name === query.analysis,
      );

      const uniqueTopic = Array.from(
        new Set(subjectFilter.map((test) => test.topic_name)),
      );

      setTopicName(uniqueTopic);

      const uniqueTest = studentTests.filter((item) =>
        topicName.some((topic) => topic === item.topic_name),
      );

      console.log({ uniqueTest });
      // const uniqueTopics = Array.from(
      //   new Set(studentTests.map((test) => test.course_name)),
      // ).map((topicId) => {
      //   const topic = studentTests.find((test) => test.course_name === topicId);
      //   return { id: topicId, name: topic?.topic_name || topicId };
      // });
    }
  }, [studentTests]);
